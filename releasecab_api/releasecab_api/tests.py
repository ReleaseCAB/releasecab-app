...
class PermissionsTestCase(TestCase):
    ...
    def test_is_tenant_owner_permission(self):
        ...
        request.user = self.user
        self.assertFalse(
            IsTenantOwnerPermission().has_permission(
                request, None))
    ...
    def test_can_create_blackouts_permission(self):
        ...
        request.user = self.user
        self.assertFalse(
            CanCreateBlackoutsPermission().has_permission(
                request, None))
    ...
    def test_can_create_releases_permission(self):
        ...
        request.user = self.user
        self.assertFalse(
            CanCreateReleasesPermission().has_permission(
                request, None))
    ...
    def test_is_tenant_owner_or_team_manager_permission(self):
        ...
        request.user = self.user
        self.assertFalse(
            IsTenantOwnerOrTeamManager().has_permission(
                request, None))
